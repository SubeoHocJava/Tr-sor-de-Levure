...
namespace Web_WineShop.Controllers
{
    public class LoginController : Controller
    {
        ...
        public IActionResult Login(string email, string password)
        {
            ...
            if (account.Role == 5) 
            { 
                return Json(new { success = true, redirectUrl = Url.Action("UserManagerment", "UserManagerment") }); 
            }
            return Json(new { success = true, redirectUrl = Url.Action("Homepage", "Homepage") });
